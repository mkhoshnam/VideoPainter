        ...
        if self.is_train:
            ...
            for example in examples:
                ...
                if frame > self.max_num_frames:
                    # If temporal_chunk_index >= 0, use a fixed 16-frame chunk
                    if getattr(self, "temporal_chunk_index", -1) is not None and self.temporal_chunk_index >= 0:
                        begin_idx = self.temporal_chunk_index * self.max_num_frames
                        # Clamp if this chunk would go past the end
                        if begin_idx + self.max_num_frames > frame:
                            begin_idx = max(0, frame - self.max_num_frames)
                    else:
                        # Original random crop behaviour
