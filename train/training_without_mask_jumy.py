...
class BalancedDemoChunkSampler(Sampler):
    def __init__(self, demo_ids, num_samples=None):
        self.demo_ids = list(demo_ids)
        self.demo_to_indices = {}
        for i, d in enumerate(self.demo_ids):
            self.demo_to_indices.setdefault(d, []).append(i)
        self.demos = list(self.demo_to_indices.keys())
        self.num_samples = int(num_samples) if num_samples is not None else len(self.demo_ids)

    def __iter__(self):
        # round-robin demos; cycle through each demo's chunks
        pools = {d: self.demo_to_indices[d][:] for d in self.demos}
        ptr = {d: 0 for d in self.demos}
        for d in self.demos:
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Fixed stride for temporal sampling.",
    )
    parser.add_argument(
        "--balance_demo_chunks",
        action="store_true",
        help="Sample chunks so each demo contributes equally (prevents long demos dominating)",
    )
    parser.add_argument(
        "--skip_frames_start",
        type=int,
...
class VideoInpaintingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # transparent_path, normal_path, id_image_path, start_frame, end_frame, fps, caption
        if self.load_mode != "online":
            raise NotImplementedError

        transparent_path, normal_path, id_image_path, start_frame, end_frame, fps, prompt = self.instance_metas[index]
        ...
        def make_abs(path):
            ...
            if not os.path.isabs(path):
                return os.path.join(str(self.instance_data_root), path)
            return path

        transparent_path = make_abs(transparent_path)
        normal_path = make_abs(normal_path)
        id_image_path = make_abs(id_image_path)

        # Helper: load [start_frame:end_frame] and resize to (self.width, self.height)
        def load_clip(path):
            ...
            video = np.stack(frames, axis=0)  # (T, H, W, 3) BGR

            # Align fps to self.fps (e.g. 30 -> 8)
            if self.fps > 0 and fps >= self.fps:
                step = max(1, fps // self.fps)
            else:
                step = 1
            video = video[::step]
            return video

        # Load transparent + normal clips
        transparent_video = load_clip(transparent_path)
        normal_video = load_clip(normal_path)

        # Ensure same temporal length
        T = min(transparent_video.shape[0], normal_video.shape[0])
        transparent_video = transparent_video[:T]
        normal_video = normal_video[:T]

        # MASK-FREE: dummy mask (all ones), no robot info
        H, W = normal_video.shape[1], normal_video.shape[2]
        binary_masks = np.ones((T, H, W), dtype=np.uint8)

        # (Optional) load init robot frame PNG, resized to same resolution
        init_image = cv2.imread(id_image_path)
        if init_image is not None:
            init_image = cv2.resize(init_image, (self.width, self.height))
        else:
            # Fallback: just use first frame of the normal video as init
            init_image = normal_video[0].copy()

        return {
            "prompt": self.id_token + str(prompt),
            "video": normal_video,  # full robot-in-scene video (TARGET)
            "transparent_video": transparent_video,  # background-only (CONDITION)
            "binary_masks": binary_masks,  # robot mask per frame
            "init_image": init_image,  # single image with robot
        }

    def _get_video_nframes(self, video_path: str) -> int:
    ...
    def _load_dataset_from_local_path(self):
        ...
        df = pd.read_csv(self.meta_file_path)

        # keep your caption filter
        df = df[df["caption"].astype(str).str.len() > 50]

        target = self.max_num_frames - ((self.max_num_frames - 1) % 4)  # 17->17
        STRIDE = int(self.fixed_stride)
        need_down = 1 + (target - 1) * STRIDE  # frames needed in DOWNsampled time
        ...
        # read rows in CSV order:
        # transparent_path, normal_path, id_image_path, start_frame, end_frame, fps, caption
        for row in df.values:
            transparent_path, normal_path, id_image_path, start_frame, end_frame, fps, prompt = row[:7]

            fps = int(fps)
            raw_start = int(start_frame)
            raw_end_csv = int(end_frame)

            abs_transparent = make_abs(transparent_path)
            raw_T = self._get_video_nframes(abs_transparent)

            raw_end = raw_end_csv if raw_end_csv > 0 else raw_T
            raw_end = min(raw_end, raw_T)
            if raw_end <= raw_start:
                continue

            # must match your downsample logic: step = fps // self.fps, but never 0
            step = max(1, fps // int(self.fps)) if int(self.fps) > 0 else 1

            raw_len = max(raw_end - raw_start, 1)
            T_down = int(math.ceil(raw_len / step))

            # FULL-COVER starts (non-overlapping windows)
            starts = list(range(0, max(T_down, 1), need_down))
            last_start = max(0, T_down - need_down)
            if last_start not in starts:
                starts.append(last_start)

            # demo id for balancing
            m = re.search(r"demo[_\-]?(\d+)", str(transparent_path))
            demo_id = int(m.group(1)) if m else 0

            for s_down in starts:
                s_raw = raw_start + s_down * step
                e_raw = raw_start + (s_down + need_down) * step
                e_raw = min(e_raw, raw_end)
                if e_raw <= s_raw:
                    continue

                expanded.append((transparent_path, normal_path, id_image_path, s_raw, e_raw, fps, prompt))
                demo_ids.append(demo_id)

        if len(expanded) == 0:
            raise ValueError("No expanded chunks created (check CSV paths / frames).")
...
class MyWebDataset():
    def __init__(self, resolution, tokenizer, random_mask, max_num_frames, max_sequence_length,
                 proportion_empty_prompts, is_train=True, mask_background=False, mix_train_ratio=0.0,
                 first_frame_gt=False, video_reshape_mode="resize", random_flip: Optional[float] = None,
                 mask_transform_prob=0.7, p_brush=0.3, p_rect=0.3, p_ellipse=0.2, p_circle=0.2, p_random_brush=0.0,
                 margin_ratio=0.1, shape_scale_min=1.1, shape_scale_max=1.5,
                 fixed_stride: int = 3):
    ...
    def __call__(self, examples):
        ...
        if self.is_train:
            ...
            for example in examples:
                ...
                video = example["video"]  # (F, H, W, 3) BGR, robot-in-scene
                transparent_video = example["transparent_video"]  # (F, H, W, 3) BGR, NO robot
                mask = example["binary_masks"]  # (F, H, W)
                init_image = example["init_image"]  # (H, W, 3) BGR DT / robot frame
                frame, height, width, c = video.shape

                # Sanity: transparent & video must have same #frames
                assert transparent_video.shape[0] == frame, "transparent_video and video must have same T"

                # Expand mask to (F, H, W, 1)
                if len(mask) > 0:
                    mask = mask[:, :, :, np.newaxis]
                else:
                    mask = np.ones_like(video[..., :1])

                target = self.max_num_frames - ((self.max_num_frames - 1) % 4)  # 17->17
                STRIDE = int(self.fixed_stride)
                need = 1 + (target - 1) * STRIDE

                # pad to need so fixed-stride indexing always valid
                if frame < need:
                    pad = need - frame
                    video = np.concatenate([video, np.repeat(video[-1:], pad, axis=0)], axis=0)
                    transparent_video = np.concatenate([transparent_video, np.repeat(transparent_video[-1:], pad, axis=0)], axis=0)
                    mask = np.concatenate([mask, np.repeat(mask[-1:], pad, axis=0)], axis=0)
                    frame = need

                idx = (np.arange(target, dtype=np.int64) * STRIDE)
                video = video[idx]
                transparent_video = transparent_video[idx]
                mask = mask[idx]
                frame = target

# Optionally perturb masks (brush/rect/etc)
                ...
                # To torch for resizing: (F, C, H, W)
                video_t = torch.from_numpy(video).permute(0, 3, 1, 2)
                transparent_t = torch.from_numpy(transparent_video).permute(0, 3, 1, 2)
                mask_t = torch.from_numpy(mask).permute(0, 3, 1, 2)

                nearest_res = self._find_nearest_resolution(video_t.shape[2], video_t.shape[3])

                # Resize videos + mask
                if self.video_reshape_mode == 'center':
                    video_resized = self._resize_for_rectangle_crop(video_t, nearest_res)
                    transparent_resized = self._resize_for_rectangle_crop(transparent_t, nearest_res)
                    mask_resized = self._resize_for_rectangle_crop(mask_t, nearest_res)
                elif self.video_reshape_mode == 'resize':
                    video_resized = torch.stack([resize(f, nearest_res) for f in video_t], dim=0)
                    transparent_resized = torch.stack([resize(f, nearest_res) for f in transparent_t], dim=0)
                    mask_resized = torch.stack([resize(f, nearest_res) for f in mask_t], dim=0)
                else:
                    raise NotImplementedError

                # Resize init_image in exactly the same way (treat as 1-frame video)
                init_t = torch.from_numpy(init_image).permute(2, 0, 1).unsqueeze(0)  # (1, C, H, W)
                if self.video_reshape_mode == 'center':
                    init_resized = self._resize_for_rectangle_crop(init_t, nearest_res)
                elif self.video_reshape_mode == 'resize':
                    init_resized = torch.stack([resize(f, nearest_res) for f in init_t], dim=0)
                else:
                    raise NotImplementedError

                # Back to numpy (F, H, W, C) / (1, H, W, C)
                video = video_resized.permute(0, 2, 3, 1).numpy()
                transparent_video = transparent_resized.permute(0, 2, 3, 1).numpy()
                mask = mask_resized.permute(0, 2, 3, 1).numpy()
                init_video = init_resized.permute(0, 2, 3, 1).numpy()  # (1, H, W, C)

                # BGR -> RGB, scale to [-1, 1]
                for i in range(frame):
                    video[i] = cv2.cvtColor(video[i], cv2.COLOR_BGR2RGB)
                    transparent_video[i] = cv2.cvtColor(transparent_video[i], cv2.COLOR_BGR2RGB)

                # init frame (1 frame)
                for i in range(init_video.shape[0]):
                    init_video[i] = cv2.cvtColor(init_video[i], cv2.COLOR_BGR2RGB)

                video = (video.astype(np.float32) / 127.5) - 1.0
                transparent_video = (transparent_video.astype(np.float32) / 127.5) - 1.0
                init_video = (init_video.astype(np.float32) / 127.5) - 1.0
                mask = mask.astype(np.float32)

                # Optional frame subsampling for mix_train_ratio
                if random.random() < self.mix_train_ratio:
                    video = video[:1]
                    transparent_video = transparent_video[:1]
                    mask = mask[:1]
                else:
                    if self.first_frame_gt:
                        # We KEEP transparent_video as pure background; only masks are used for extra loss.
                        # So we do NOT overwrite transparent_video[0] with video[0].
                        # If you ever want to sync mask semantics, do it here.
                        pass

                # Convert to (F, C, H, W) tensors and accumulate
                pixel_values.append(torch.tensor(video).permute(0, 3, 1, 2))
                conditioning_pixel_values.append(torch.tensor(transparent_video).permute(0, 3, 1, 2))
                masks.append(torch.tensor(mask).permute(0, 3, 1, 2))
                init_pixel_values.append(torch.tensor(init_video).permute(0, 3, 1, 2))
            ...
            for example in examples:
                caption = example["prompt"]
                video = example["video"]  # frame, height, width, c (TARGET robot video)
                transparent_video = example["transparent_video"]  # frame, height, width, c (BACKGROUND)
                init_image = example["init_image"]  # (H, W, 3) BGR DT / robot frame
                mask = example["binary_masks"]  # frame, height, width
                frame, height, width, c = video.shape
                assert mask.shape[0] == frame, "mask shape is not equal to video shape"
                assert transparent_video.shape[0] == frame, "transparent_video shape is not equal to video shape"
                ...
                transparent_video_ = [
                    Image.fromarray(cv2.cvtColor(transparent_video[i], cv2.COLOR_BGR2RGB))
                    for i in range(frame)
                ]
                masked_video_ = [Image.fromarray(cv2.cvtColor(masked_video[i], cv2.COLOR_BGR2RGB)) for i in
                                 range(frame)]

                # Single-frame DT PNG as conditioning image
                init_pil = Image.fromarray(cv2.cvtColor(init_image, cv2.COLOR_BGR2RGB))

                pixel_values.append(video_)  # TARGET robot video
                conditioning_pixel_values.append(transparent_video_)  # BACKGROUND video
                masks.append(mask_)
                init_pixel_values.append([init_pil])  # shape: [F=1]
...
def log_validation(
    ...
    for _ in range(args.num_validation_videos):
        ...
        original_video = pipe.video_processor.preprocess_video(pipeline_args['video'][0], height=video.shape[1],
                                                               width=video.shape[2])
        masks = pipe.masked_video_processor.preprocess_video(pipeline_args['masked_video'][0], height=video.shape[1],
                                                             width=video.shape[2])
        ...
        if tracker.name == "wandb":
            ...
            for i, video in enumerate(videos):
                ...
                filename = os.path.join(args.output_dir,
                                        f"{phase_name}_video_{i}_{pipeline_args['replace_gt']}_{prompt}.mp4")
...
def main(args):
    ...
    # We only train the additional context encoder branch (mask-free)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for model in models:
                ...
                if isinstance(unwrapped, CogvideoXBranchModel):
                    unwrapped.save_pretrained(
                        os.path.join(output_dir, "branch"), safe_serialization=True, max_shard_size="5GB"
                    )
                else:
                    logger.warning(f"Skipping unexpected model type during save: {unwrapped.__class__.__name__}")
    ...
    def load_model_hook(models, input_dir):
        while len(models) > 0:
            ...
            if isinstance(unwrapped, CogvideoXBranchModel):
                ...
                logger.warning(f"Skipping unexpected model type during load: {unwrapped.__class__.__name__}")

        if args.mixed_precision == "fp16":
            cast_training_params([branch])
    ...
    if args.mixed_precision == "fp16":
        # only upcast trainable parameters (trainable modules) into fp32
    ...
    params_to_optimize = [
        {"params": branch_parameters, "lr": args.learning_rate},
    ]
    ...
    logger.info(
        f"use_deepspeed_optimizer: {use_deepspeed_optimizer}, use_deepspeed_scheduler: {use_deepspeed_scheduler}")
    ...
    if args.balance_demo_chunks:
        train_sampler = BalancedDemoChunkSampler(train_dataset._demo_ids, num_samples=len(train_dataset))
        train_shuffle = False
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                if global_step < 1:
                    ...
                    conditioning_pixel_values = (batch["conditioning_pixel_values"][0, 0].permute(1, 2,
                                                                                                  0).cpu().numpy() + 1) * 127.5
                    ...
                    combined_image = np.hstack((pixel_values, conditioning_pixel_values))
                    cv2.imwrite(f'{args.runs_name}_training_sample_1_{global_step}.png', combined_image)

                    if batch["pixel_values"].shape[1] > 1:
                        pixel_values = (batch["pixel_values"][0, 1].permute(1, 2, 0).cpu().numpy() + 1) * 127.5
                        conditioning_pixel_values = (batch["conditioning_pixel_values"][0, 1].permute(1, 2,
                                                                                                      0).cpu().numpy() + 1) * 127.5
                ...
                # Use DT init image with the robot
                images = batch["init_pixel_values"].permute(0, 2, 1, 3, 4)  # [B, C, F=1, H, W]
                ...
                if global_step < 1 and accelerator.is_local_main_process:
                    # before noise
                    init_before = (images[0, :, 0].permute(1, 2, 0).detach().cpu().numpy() + 1) * 127.5
                    init_before = init_before.clip(0, 255).astype(np.uint8)
                    init_before = cv2.cvtColor(init_before, cv2.COLOR_BGR2RGB)
                    cv2.imwrite(f"{args.runs_name}_init_before_noise_{global_step}.png", init_before)

                    # after noise
                    init_after = (noisy_images[0, :, 0].permute(1, 2, 0).detach().cpu().numpy() + 1) * 127.5
                    init_after = init_after.clip(0, 255).astype(np.uint8)
                    init_after = cv2.cvtColor(init_after, cv2.COLOR_BGR2RGB)
                    cv2.imwrite(f"{args.runs_name}_init_after_noise_{global_step}.png", init_after)

                # noisy_images - initial image of the scene with the robot
                ...
                # "piel_values" - target video with the robot
                # model_input - target video with the robot
                model_input = vae.encode(
                    batch["pixel_values"].permute(0, 2, 1, 3, 4).to(dtype=weight_dtype)).latent_dist.sample()
                ...
                # 'conditioning_pixel_values' - transparent video without robot
                # conditioning_latents - transparent video without robot
                conditioning_latents = vae.encode(batch["conditioning_pixel_values"].permute(0, 2, 1, 3, 4).to(
                    dtype=weight_dtype)).latent_dist.sample()
                conditioning_latents = conditioning_latents.permute(0, 2, 1, 3,
                                                                    4) * vae.config.scaling_factor  # [B, F, C, H, W]
                conditioning_latents = conditioning_latents.to(memory_format=torch.contiguous_format,
                                                               dtype=weight_dtype)

                b, f, c, h, w = conditioning_latents.shape
                dummy_channel = torch.zeros(
                    b, f, 1, h, w,
                    device=conditioning_latents.device,
                    dtype=conditioning_latents.dtype,
                )
                conditioning_latents = torch.cat([conditioning_latents, dummy_channel], dim=2)

                # Time-align image_latents (F_img) with the video latents (F_vid)
                video_frames = model_input.shape[1]  # F_vid (frames in target video)
                image_frames = image_latents.shape[1]  # F_img (usually 1)

                if image_frames < video_frames:
                    padding_shape = (
                    ...
                    # image_latents - initial image of the scene with the robot
                    image_latents = torch.cat([image_latents, latent_padding], dim=1)
                elif image_frames > video_frames:
                    # Just in case, truncate 
                    image_latents = image_latents[:, :video_frames]
                ...
                # Mask-free: ignore batch['masks'], only use init image + transparent video as conditioning
                ...
                branch_block_samples = branch(
                    ...
                    mask_add=False,  # force mask-free
                    wo_text=args.wo_text,
                    return_dict=False,
                )[0]
                ...
                # Mask-free reconstruction loss
                loss = torch.mean((weights * (model_pred - target) ** 2).reshape(batch_size, -1), dim=1)
                loss = loss.mean()

                inpainting_loss = torch.tensor(0.0, device=loss.device)

                accelerator.backward(loss)

                if accelerator.sync_gradients:
                    params_to_clip = list(branch.parameters())
            ...
            logs = {"loss": loss.detach().item(), "inpainting_loss": inpainting_loss.detach().item(),
            ...
            if args.validating_steps > 0 and (global_step % args.validating_steps == 0):
                ...
                for step, batch in enumerate(validation_dataloader):
                    pipeline_args = {
                        ...
                        "height": args.height,
                        "width": args.width,
                        "image": batch["init_pixel_values"][0][0],
                        "video": batch["conditioning_pixel_values"],
                        "prompt": batch["input_ids"],
                        "masked_video": batch["masks"],
                        "num_frames": np.array(batch['conditioning_pixel_values'][0]).shape[0],
                        "strength": 1.0,
                        "mask_background": args.mask_background,
                        "add_first": args.add_first,
                        "wo_text": args.wo_text,
                    ...
                    if vali_num >= 2:
                        break
        if args.validating_steps > 0 and ((epoch + 1) % args.validation_epochs == 0):
            try:
                pipe = CogVideoXI2VDualInpaintPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    transformer=unwrap_model(transformer),
