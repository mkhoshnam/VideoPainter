...
class VideoInpaintingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Compute robot mask via frame-wise difference (1 where robot is)
        diff = cv2.absdiff(normal_video, transparent_video)  # (T, H, W, 3)
        diff_gray = diff.max(axis=-1)  # (T, H, W)
        binary_masks = (diff_gray > 15).astype(np.uint8)  # 1 where robot moved

        # Limit to max_num_frames used by training
        T = min(T, self.max_num_frames)
        normal_video = normal_video[:T]
        transparent_video = transparent_video[:T]
        binary_masks = binary_masks[:T]
    ...
    def __call__(self, examples):
        ...
        if self.is_train:
            ...
            for example in examples:
                ...
                # Temporal cropping / padding (apply to all three: video, transparent, mask)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                print("noisy_model_input.shape =", noisy_model_input.shape)          # transformer input tensor
                print("latent_frames (F) =", noisy_model_input.shape[1])             # time steps seen by transformer
                print("latent_channels (C) =", noisy_model_input.shape[2])           # channels seen by transformer

                # (optional, to connect back to pixel frames)
                print("conditioning_pixel_values.shape =", batch["conditioning_pixel_values"].shape)
                print("pixel_frames =", batch["conditioning_pixel_values"].shape[1]) # 13 or 16
