...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="If >=0, always crop the same 16-frame temporal chunk index from each video (0 = first chunk).",
    )
    parser.add_argument(
        "--skip_frames_start",
        type=int,
...
class MyWebDataset():
    def __init__(self, resolution, tokenizer, random_mask, max_num_frames, max_sequence_length,
        ...
        self.temporal_chunk_index = temporal_chunk_index
    ...
    def __call__(self, examples):
        ...
        if self.is_train:
            ...
            for example in examples:
                ...
                # Temporal sampling (ONE policy): cover the whole demo with a fixed-length clip (jumpy).
                # We must satisfy downstream T % 4 == 1. For max_num_frames=16 -> target=13 frames.
                target = self.max_num_frames - ((self.max_num_frames - 1) % 4)
                if frame >= target:
                    idx = np.linspace(0, frame - 1, num=target, dtype=np.int64)
                    video = video[idx]
...
def main(args):
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=MyWebDataset(
            ...
            temporal_chunk_index=args.temporal_chunk_index,
            is_train=True,
        ),
