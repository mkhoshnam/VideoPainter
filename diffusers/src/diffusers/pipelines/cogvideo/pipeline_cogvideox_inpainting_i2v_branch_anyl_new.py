...
class CogVideoXI2VDualInpaintAnyLPipelineNew(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        self.masked_video_processor = VideoProcessor(vae_scale_factor=self.vae_scale_factor_spatial, do_normalize=False,
    ...
    def prepare_mask_latents(
        ...
        mask = torch.nn.functional.interpolate(
            mask,
            size=((mask.shape[-3] - 1) // self.vae_scale_factor_temporal + 1, height // self.vae_scale_factor_spatial,
                  width // self.vae_scale_factor_spatial)
        )
    ...
    def __call__(
        ...
        if stride < num_frames:
            num_frame_latents = ((num_frames - 1) // self.vae_scale_factor_temporal + 1) * n_windows - (
                        n_windows - 1) * ((num_frames - stride) // self.vae_scale_factor_temporal + 1)
        ...
        for window_idx in range(n_windows):
            ...
            if window_idx == 0:
                ...
                if -int((num_frames - stride) // self.vae_scale_factor_temporal) < 0:
                    image_ = latents[:, -int((num_frames - stride) // self.vae_scale_factor_temporal) - 1:-int(
                        (num_frames - stride) // self.vae_scale_factor_temporal) + 1 - 1]
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    # branch block
                    latent_branch_input = masked_video_latents  # PATCH: mask-free (no mask channel)
                    ...
                    branch_block_samples = [block_sample.to(dtype=prompt_embeds.dtype) for block_sample in
                    ...
                    if replace_gt:
                        ...
                        if i < len(timesteps) - 1:
                            ...
                            init_latents_proper = self.scheduler.add_noise(
                                init_latents_proper.to(device), noise.to(device),
                                torch.tensor([noise_timestep]).to(device)
                            ).to(device)
            ...
            for i in range(latents.shape[1]):
                ...
                if window_idx > 0 and stride < num_frames:
                    compressed_start_frame -= (int((
                                                               num_frames - stride) // self.vae_scale_factor_temporal) + 1) * window_idx
